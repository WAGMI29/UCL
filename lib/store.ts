...
// ─── Write Queue (retry failed Supabase writes on reconnect) ──

const WRITE_QUEUE_KEY = "@parables_write_queue";
...
async function enqueueWrite(write: Omit<QueuedWrite, "timestamp">) {
  try {
    const raw = await AsyncStorage.getItem(WRITE_QUEUE_KEY);
    const queue: QueuedWrite[] = raw ? JSON.parse(raw) : [];
    queue.push({ ...write, timestamp: Date.now() });
  ...
  try {
    ...
    for (const item of queue) {
      const { error } = await supabase
        .from(item.table)
        .upsert(item.data, item.onConflict ? { onConflict: item.onConflict } : undefined);
...
async function safeUpsert(
  ...
  if (error) {
    await enqueueWrite({ table, data, onConflict });
  }
}
  ...
  persist(
    (set, get) => ({
      ...
      updateProfile: async (updates) => {
        ...
        await safeUpsert("profiles", { id: user.id, ...updates });
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        bibleFamiliarity: state.bibleFamiliarity,
